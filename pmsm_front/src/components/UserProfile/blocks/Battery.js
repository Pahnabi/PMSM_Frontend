...
const BatteryComponent = () => {
  ...
  const [statusText, setStatusText] = useState('Low battery <i class="ri-plug-line"></i>');
  const [batteryLiquidHeight, setBatteryLiquidHeight] = useState('0%');
  const [batteryLiquidClass, setBatteryLiquidClass] = useState('gradient-color-red');
  ...
  const updateBatteryStatus = (charge, isCharging) => {
    ...
    if (level === 100) {
      setStatusText('Full battery <i class="ri-battery-2-fill green-color"></i>');
      setBatteryLiquidHeight('103%');
    } else if (level <= 20 && !isCharging) {
      setStatusText('Low battery <i class="ri-plug-line animated-red"></i>');
    } else if (isCharging) {
      setStatusText('Charging... <i class="ri-flashlight-line animated-green"></i>');
    } else {
      setStatusText('');
    }
